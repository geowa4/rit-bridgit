...
public class EditScopeParser
{
	...
	public ArrayList<ArrayList<String>> parseScope(String p_ProgramText)
	{
		...
		try
		{
			...
			Pattern variablePattern = Pattern.compile("\\s*var\\s+\\w*\\s*:\\s*\\w*\\s*");
			Pattern constantPattern = Pattern.compile("\\s*constant\\s+\\w*\\s*:\\s*\\w*\\s*");
			Pattern functionPattern = Pattern.compile("\\s*function\\s+\\w*\\s*(.*)\\s*:\\s*\\w*\\s*");
		...
		{
			// Print out the error
			System.err.println("Failed scope parsing of program text editor. See the following: ");
			e.printStackTrace();
		}
